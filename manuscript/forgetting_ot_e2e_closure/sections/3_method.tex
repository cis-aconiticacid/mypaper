...
let $n_t$ be the batched-iteration number of the first configured verification at which the problem passes the two-marginal verifier. DrainSinkhorn then writes its
...
screening invokes the two-marginal verifier unnecessarily; a false-negative
screening delays removal. We measure these effects by replaying the screened
results and screening delays removal. We measure these effects by replaying the screened
results and evaluate each with the verifier in \cref{sec:experiments}. \cref{sec:experiments}.
