...
The performance model uses a width-cost function for a specified device, kernel, problem shape, and precision. The reported scheduling-wave behavior describes the measured A100 Triton configuration. Applying the model to another backend requires corresponding width-cost and control-cost calibration, including its tail path. A held-out application prediction would test these calibrated terms jointly. Offline completion counts also support analysis of batch grouping; using estimated counts for online grouping remains a separate scheduling problem.
