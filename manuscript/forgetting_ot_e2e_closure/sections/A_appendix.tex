...
In \cref{tab:phase-costs}, ``Batched verification'' is the batched marginal audit, implemented as the row-only screen in these two arms. ``Verifier'' is the separate upstream two-marginal plan-apply check on selected candidates. ``Correction'' covers Sinkhorn updates, and the last column records state freezing or physical selection. Each column is summarized independently; their medians are not substituted for the paired whole-path timing ratio.
