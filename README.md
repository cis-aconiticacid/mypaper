...
- [Batching implementation audit](BATCHING_IMPLEMENTATION_AUDIT.md) — pinned source-level comparison plus direct modifications of POT and GeomLoss, with the GeomLoss residual extension stated explicitly.
...
avoiding later work on verified-complete problems. It reports direct POT
modification against unmodified POT, and full-width versus compacted execution
inside GeomLoss's own loop after adding the same residual refinement to both
controls. The complete inventory retains the earlier release feasibility,
cross-backend measurements and failures. See the source audit and detailed
navigator for evidence and scope.
